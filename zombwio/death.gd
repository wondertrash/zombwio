...
func show_death_screen(time: float):
	...
	death_text.position = Vector2(viewport_size.x * 0.5 - 110, viewport_size.y * 0.15)
	...
	time_text.position = Vector2(viewport_size.x * 0.5 - 120, viewport_size.y * 0.3)
	...
	stats_text.position = Vector2(viewport_size.x * 0.5 - 120, viewport_size.y * 0.4)
	stats_text.add_theme_font_size_override("font_size", 18)
	add_child(stats_text)
	var restart_btn = Button.new()
	restart_btn.text = "Restart"
	restart_btn.position = Vector2(viewport_size.x * 0.5 - 70, viewport_size.y * 0.65)
	restart_btn.size = Vector2(140, 60)
	restart_btn.pressed.connect(_restart)
	add_child(restart_btn)
	var menu_btn = Button.new()
	menu_btn.text = "Main Menu"
	menu_btn.position = Vector2(viewport_size.x * 0.5 - 70, viewport_size.y * 0.8)
